...
impl PdfReporter {
    ...
    pub fn generate(&self, summary: &ScanSummary, output_path: &Path) -> Result<()> {
        ...
        lines.push(format!(
            "Data Scanned:   {}",
            Self::format_bytes(summary.bytes_scanned)
        ));
    ...
    fn write_pdf(&self, file: &mut File, lines: &[String]) -> Result<()> {
        ...
        writeln!(file, "<< /Size {} /Root 1 0 R >>", obj_positions.len() + 1)?;
...
mod tests {
    ...
    fn test_escape_pdf_string() {
        ...
        assert_eq!(PdfReporter::escape_pdf_string("(test)"), "\\(test\\)");
        assert_eq!(
            PdfReporter::escape_pdf_string("back\\slash"),
            "back\\\\slash"
        );
    }
